    ...
    public class ScheduleService
    {
            ...
            if (model.ApplicationId > 0)
            {
                try
                {
                    ...
                    if (app != null)
                    {
                        ...
                        var schFlow = await _flow.GetWorkFlow(Enum.GetName(typeof(AppActions), AppActions.ScheduleInspection), user, app.Facility.VesselTypeId, app.ApplicationTypeId);
                        if (schFlow != null)
                        {
            ...
            try
            {
                ...
                if (appointment != null)
                {
                    var schFlow = model.Act.Equals(Enum.GetName(typeof(AppActions), AppActions.ApproveInspection))
                        ? await _flow.GetWorkFlow(Enum.GetName(typeof(AppActions), AppActions.ApproveInspection), user, appointment.Application.Facility.VesselTypeId, app.ApplicationTypeId)
                        : await _flow.GetWorkFlow(Enum.GetName(typeof(AppActions), AppActions.RejectInspection), user, appointment.Application.Facility.VesselTypeId, app.ApplicationTypeId);
                    if (schFlow != null)
                    {
        ...
        public async Task<ApiResponse> AcceptSchedule(ScheduleViewModel model)
        {
            try
            {
                ...
                if (appointment != null)
                {
                    var schFlow = await _flow.GetWorkFlow(Enum.GetName(typeof(AppActions), AppActions.AcceptInspection), user, app.Facility.VesselTypeId, app.ApplicationTypeId);
                    if (schFlow != null)
                    {
